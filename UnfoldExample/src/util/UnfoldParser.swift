...
class UnfoldParser{
     ...
     */
    static func unfoldable(parent:UnFoldable, _ path:[Int]) throws -> UnFoldable{//TODO: ⚠️️ rename to paths?
        ...
        for subView in parentView.subviews{
            if let subV = subView as? UnFoldable  {
                if UnfoldAsserter.isMatch(subV, path[0]){//asserts element.id
                    if path.count > 1 {
                        // 🏀 Figure out how array without first
                     let pathsSansFirst = Array(path[1..<path.count])
                        if let unfoldable = try? unfoldable(parent:subV, pathsSansFirst) {return unfoldable}/*removes first item in path*/ //retrieve(sub, path)
                    }else{
                        return subV
                    }
                }
     ...
     */
    static func unfoldable<T>(parent:UnFoldable, path:[Int]) throws -> T {//TODO: ⚠️️ rename to paths?
     ...
     */
    static func value<T>(_ view:UnFoldable, _ path:[Int]) throws -> T {//TODO: ⚠️️ rename to paths?
